...
namespace dqm4hep
{
  ...
  StatusCode TimeCorrelation::readSettings(const TiXmlHandle xmlHandle)
  {
    ...
    RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, 
                            DQMXmlHelper::readParameterValue(xmlHandle, "bifoffest", m_bifoffset));
    ...
    m_pTDCvsTime_c = NULL;
    m_pTDCvsTime_d = NULL;
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle, "TDCvsTime_a", m_pTDCvsTime_a));
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle, "TDCvsTime_b", m_pTDCvsTime_b));
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle, "TDCvsTime_c", m_pTDCvsTime_c));
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle, "TDCvsTime_d", m_pTDCvsTime_d));
		...
		float Time_BIF = (float)((RelativeTime - m_bifoffset)%BIF_BINNING * BIF_RESOLUTION);
		  ...
		  for (unsigned int iraw = 0; iraw < it->second.size(); iraw++ )
			...
			if(it->first == BXID_BIF && hitbit_adc==hitbit_tdc && gainbit_adc==gainbit_tdc && hitbit_adc==1 && adc > 0) {
			  ...
			  if(it->second.at(iraw)[ChipIDIndex]==m_ChipNr_a && f==m_ChannelNr_a) {
			    Int_t pointID = m_pTDCvsTime_a->get<TGraph>()->GetN();
			    m_pTDCvsTime_a->get<TGraph>()->SetPoint(pointID, Time_BIF, tdc );
			  }
			  if(it->second.at(iraw)[ChipIDIndex]==m_ChipNr_b && f==m_ChannelNr_b) {
			    Int_t pointID = m_pTDCvsTime_b->get<TGraph>()->GetN();
			    m_pTDCvsTime_b->get<TGraph>()->SetPoint(pointID, Time_BIF, tdc );
			  }
			  if(it->second.at(iraw)[ChipIDIndex]==m_ChipNr_c && f==m_ChannelNr_c) {
			    Int_t pointID = m_pTDCvsTime_c->get<TGraph>()->GetN();
			    m_pTDCvsTime_c->get<TGraph>()->SetPoint(pointID, Time_BIF, tdc );
			  }
			  if(it->second.at(iraw)[ChipIDIndex]==m_ChipNr_d && f==m_ChannelNr_d) {
			    Int_t pointID = m_pTDCvsTime_d->get<TGraph>()->GetN();
			    m_pTDCvsTime_d->get<TGraph>()->SetPoint(pointID, Time_BIF, tdc );
			  }
  ...
  StatusCode TimeCorrelation::startOfRun(DQMRun *pRun)
  {
    ...
    pRun->getParameter("ChipNr_a",m_ChipNr_a) ;
    pRun->getParameter("ChipNr_b",m_ChipNr_b) ;
    pRun->getParameter("ChipNr_c",m_ChipNr_c) ;
    pRun->getParameter("ChipNr_d",m_ChipNr_d) ;
