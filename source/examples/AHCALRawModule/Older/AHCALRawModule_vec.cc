...
namespace dqm4hep
{
  ...
  StatusCode AHCALRawModule::readSettings(const TiXmlHandle xmlHandle)
  {
    ...
    m_pADC_hitbit0.resize(400)
    for(int j = 0; j < 400; j++) {
      m_pADC_hitbit0[j].resize(36);
    }
    ...
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle,
									       "ADC_hb0_ch1", m_pADC_hitbit0_ch1));

    m_pADC_hitbit1_ch1 = NULL;
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle,
									       "ADC_hb1_ch1", m_pADC_hitbit1_ch1));

    m_pTDC_hitbit0_ch1 = NULL;
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle,
									       "TDC_hb0_ch1", m_pTDC_hitbit0_ch1));

    m_pTDC_hitbit1_ch1 = NULL;
    RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle,
									       "TDC_hb1_ch1", m_pTDC_hitbit1_ch1));
    // Histograms for each channel and chip combination
    // We need to access the number of chips, their IDs and the number of channels per chip before we have access to this
    // information from the SLCIO file. We can define these manually here but it's not a great solution.

    // We have to use two for loops, because we are skipping Module 13 and the ChipIDs will go from 169-184, then 201-232.

    // 3 layers, 16 chips per layer (12,14,15)
    // Module 15: 217-232
    // Module 14: 201-216
    // Module 13: 185-200
    // Module 12: 169-184

    for(int j = 169; j < 184+1; j++) {		// j is the ChipId, k is the channel number
      for(int k = 0; k < 36; k++) {
	m_pADC_hitbit0[j][k] = NULL;
	RETURN_RESULT_IF(STATUS_CODE_SUCCES, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle,
										  TString::format("ADC_hb0_Chip%d_Chan%d",j,k), m_pADC_hitbit0[j][k]));
      }
    }

    for(int j = 201; j < 232+1; j++) {		// j is the ChipId, k is the channel number
      for(int k = 0; k < 36; k++) {
	m_pADC_hitbit0[j][k] = NULL;
	RETURN_RESULT_IF(STATUS_CODE_SUCCES, !=, DQMXmlHelper::bookMonitorElement(this, xmlHandle,
										  TString::format("ADC_hb0_Chip%d_Chan%d",j,k), m_pADC_hitbit0[j][k]));
		...
		for(int e=0 ; e<nElements ; e++)
		  {
			...
			if( (hitbit_tdc[f] != hitbit_adc[f]) || (gainbit_tdc[f] != gainbit_adc[f]) ) {
			  nEventsRejected++; 
			  LOG4CXX_INFO( dqmMainLogger , "Incoherent bit information, happened " << nEventsRejected << " times" ); 
			  ...
			  continue; 	
			}	// Rejecting mismatched hits


			//std::cout << "==Before filling histos==" << std::endl;
			...
			if(f==0 && (pAHCALRaw->getIntVal(ChipIDIndex) == 129)) {		// Channel 0, all chips
			  //std::cout << "==Channel filling started==" << std::endl;
			  //std::cout << "==ADC: " << adc[f] << "; with HitBit = " << hitbit_adc[f] << " ==" << std::endl;
			  if(hitbit_adc[f]==0) m_pADC_hitbit0_ch1->get<TH1I>()->Fill(adc[f]);
			  if(hitbit_adc[f]==1) m_pADC_hitbit1_ch1->get<TH1I>()->Fill(adc[f]);

			  //std::cout << "==Channel filled ADC==" << std::endl;

			  //std::cout << "==TDC: " << tdc[f] << " ==" << std::endl;

			  if(hitbit_tdc[f]==0) m_pTDC_hitbit0_ch1->get<TH1I>()->Fill(tdc[f]);
			  if(hitbit_tdc[f]==1) m_pTDC_hitbit1_ch1->get<TH1I>()->Fill(tdc[f]);
			  //std::cout << "==Channel filling finished==" << std::endl;
			}


			if (hitbit_adc[f]==0) m_pADC_hitbit0[ChipID][ChannelNo]->get<TH1I>()->Fill(adc[f])


						//std::cout << "==After filling histos (by single channel)==" << std::endl;

						//std::cout << "==LOOP No. " << f << " END==" << std::endl;
						}
